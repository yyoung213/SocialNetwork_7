"""
Pajek Bipartite 파일을 기반으로 Skill-Skill One-mode Projection 네트워크 생성

입력: posting_skill_bipartite_2mode.net (Pajek bipartite 형식)
출력: skill_skill_network.net (Pajek one-mode 형식)
"""
...
def create_skill_skill_network(bipartite_G, skill_nodes):
    """Bipartite 네트워크에서 Skill-Skill One-mode Projection을 생성합니다."""
    print(f"[3단계] Skill-Skill One-mode Projection 생성")
    
    # 스킬 노드 ID 리스트
    skill_node_ids = list(skill_nodes.keys())
    
    # Weighted projection 생성
    skill_skill_G = nx.bipartite.weighted_projected_graph(bipartite_G, skill_node_ids)
    ...
    return skill_skill_G, skill_nodes


def save_pajek_network(G, skill_nodes, output_file):
    """Skill-Skill 네트워크를 Pajek 형식으로 저장합니다."""
    print(f"[4단계] Pajek .net 파일 생성: {output_file}")
    
    # 노드 ID 매핑 (스킬 이름 -> 1부터 시작하는 숫자 ID)
    sorted_skills = sorted(G.nodes())
    skill_to_id = {skill: idx + 1 for idx, skill in enumerate(sorted_skills)}
    
    with open(output_file, 'w', encoding='utf-8') as f:
        # Vertices 섹션
        f.write(f"*Vertices {G.number_of_nodes()}\n")
        for skill in sorted_skills:
            node_id = skill_to_id[skill]
            skill_name = skill_nodes.get(skill, skill) if isinstance(skill, int) else skill
            # Pajek 형식: id "label"
            label_escaped = str(skill_name).replace('"', '\\"')
...
def main():
    """메인 함수"""
    print("=" * 70)
    print("Skill-Skill One-mode Projection 네트워크 생성 (Pajek 형식)")
    print("=" * 70)
    
    # 경로 설정
    script_dir = os.path.dirname(os.path.abspath(__file__))
    input_file = os.path.join(script_dir, 'posting_skill_bipartite_2mode.net')
    output_file = os.path.join(script_dir, 'skill_skill_network.net')
    ...
    # 3단계: Skill-Skill One-mode Projection 생성
    skill_skill_G, skill_node_dict = create_skill_skill_network(bipartite_G, skill_nodes)
    
    # 4단계: Pajek 형식으로 저장
    # skill_skill_G의 노드는 원래 node_id이므로, skill_nodes에서 이름을 가져와야 함
    # 하지만 projection 후에는 스킬 이름이 노드가 되었을 수도 있음
    # 실제로는 skill_node_dict를 사용하여 매핑
    save_pajek_network(skill_skill_G, skill_nodes, output_file)
