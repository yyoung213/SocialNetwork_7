...
def save_pajek_network(G, skill_nodes, output_file):
    ...
    # G의 노드는 원래 node_id이므로, skill_nodes에서 이름을 가져와야 함
    sorted_skill_ids = sorted(G.nodes())
    skill_to_id = {skill_id: idx + 1 for idx, skill_id in enumerate(sorted_skill_ids)}
    
    with open(output_file, 'w', encoding='utf-8') as f:
        # Vertices 섹션
        f.write(f"*Vertices {G.number_of_nodes()}\n")
        for skill_id in sorted_skill_ids:
            node_id = skill_to_id[skill_id]
            skill_name = skill_nodes.get(skill_id, str(skill_id))
            # Pajek 형식: id "label"
